...
contract LauncherPlugin is ILauncherPlugin {
    ...
    modifier onlyAuthority() {
        ...
        require(authorityMap[msg.sender] || msg.sender == accessHub, Errors.NOT_AUTHORITY());
        _;
    }

    modifier onlyOperator() {
        /// @dev redundant `operator` address put here as a safeguard for input errors on transferring roles
        require(msg.sender == accessHub || msg.sender == operator, Errors.NOT_OPERATOR());
        _;
    }
    ...
    function migratePool(address _oldPool, address _newPool) external {
        ...
        require(msg.sender == accessHub || msg.sender == operator, Errors.NOT_AUTHORIZED(msg.sender));
