...
contract Pair is IPair, ERC20, ReentrancyGuard {
    ...
    function initialize(address _token0, address _token1, bool _stable) external onlyFactory {
        ...
        if (_stable) {
            ...
            __symbol = string(
                string.concat("cAMM V2 - ", IERC20Extended(token0).symbol(), "/", IERC20Extended(token1).symbol())
            );
        } else {
            __name = string(
                string.concat(
                    "Shadow V2 Legacy Volatile - ",
                    IERC20Extended(token0).symbol(),
                    "/",
                    IERC20Extended(token1).symbol()
                )
            ...
            __symbol = string(
                string.concat("vAMM V2 - ", IERC20Extended(token0).symbol(), "/", IERC20Extended(token1).symbol())
            );
        }
            ...
            if (_kLast != 0) {
                ...
                if (stable) {
                    ...
                    if (k > _kLast) {
                        ...
                        uint256 denominator = ((10_000 * 1e18) - (_feeSplit * (1e18 - fourthRoot_e18)));
