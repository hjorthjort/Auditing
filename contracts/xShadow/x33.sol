...
contract x33 is ERC4626, IX33, ReentrancyGuard {
    ...
    constructor(address _operator, address _accessHub, address _xShadow, address _voter, address _voteModule)
        ERC20("Shadow Liquid Staking Token", "x33")
        ERC4626(IERC20(_xShadow))
    {
        operator = _operator;
    ...
    function claimRebase() external onlyOperator {
        ...
        require(block.timestamp > voteModule.periodFinish(), Errors.REBASE_IN_PROGRESS());
    ...
    function swapIncentiveViaAggregator(AggregatorParams calldata _params) external nonReentrant onlyOperator {
        ...
        require(_params.tokenIn != address(shadow), Errors.FORBIDDEN_TOKEN(address(shadow)));
        ...
        require(diffShadow >= _params.minAmountOut, Errors.AMOUNT_OUT_TOO_LOW(diffShadow));
        /// @dev prevent any holding xshadow on x33 to be manipulated (under any circumstance)
        require(xShadowBalanceAfterSwap == xShadowBalanceBeforeSwap, Errors.FORBIDDEN_TOKEN(address(shadow)));
    ...
    function rescue(address _token, uint256 _amount) external nonReentrant onlyAccessHub {
        ...
        require(xShadow.allowance(_token, address(this)) == 0, Errors.FORBIDDEN_TOKEN(address(xShadow)));
        require(totalAssets() == snapshotxShadowBalance, Errors.FORBIDDEN_TOKEN(address(xShadow)));
    }
