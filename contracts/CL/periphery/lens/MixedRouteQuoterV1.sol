...
import 'contracts/CL/core/interfaces/IShadowV3Pool.sol';
import 'contracts/CL/core/interfaces/IShadowV3Factory.sol';
...
import 'contracts/CL/universalRouter/modules/uniswap/v2/ShadowLegacyLibrary.sol';
...
contract MixedRouteQuoterV1 is IMixedRouteQuoterV1, IUniswapV3SwapCallback {
    ...
    using PoolTicksCounter for IShadowV3Pool;
    ...
    constructor(address _factory, address _pairFactory) {
        factory = _factory;
        contractDeployer = IShadowV3Factory(factory).shadowV3PoolDeployer();
        pairFactory = _pairFactory;
        initCodeHash = IPairFactory(_pairFactory).pairCodeHash();
    }

    function getPool(address tokenA, address tokenB, int24 tickSpacing) private view returns (IShadowV3Pool) {
        return IShadowV3Pool(PoolAddress.computeAddress(contractDeployer, PoolAddress.getPoolKey(tokenA, tokenB, tickSpacing)));
    }
    ...
    function getPairAmountOut(
        ...
        address pair = ShadowLegacyLibrary.pairFor(pairFactory, initCodeHash, tokenIn, tokenOut, stable);
        uint256 fee = IPairFactory(pairFactory).pairFee(pair);
        (uint256 reserveIn, uint256 reserveOut, uint256 decimalsIn, uint256 decimalsOut) = ShadowLegacyLibrary.getReserves(
            pairFactory,
            initCodeHash,
            tokenIn,
            tokenOut,
        ...
        amountIn -= (amountIn * fee) / 1_000_000;
        return ShadowLegacyLibrary.getAmountOut(amountIn, reserveIn, reserveOut, stable, decimalsIn, decimalsOut);
    }
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory path) external view override {
        ...
        IShadowV3Pool pool = getPool(tokenIn, tokenOut, tickSpacing);
        ...
        returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)
    {
        ...
        IShadowV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.tickSpacing);
