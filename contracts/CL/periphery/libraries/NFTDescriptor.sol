...
library NFTDescriptor {
    ...
    function generateDescriptionPartOne(
        ...
        return
            string(
                abi.encodePacked(
                    'This NFT represents a liquidity position in a Shadow V3 ',
                    quoteTokenSymbol,
                    '-',
                    baseTokenSymbol,
                    ' pool. ',
    ...
    struct DecimalStringParams {
        /// @dev significant figures of decimal
        uint256 sigfigs;
        /// @dev length of decimal string
        uint8 bufferLength;
        /// @dev ending index for significant figures (funtion works backwards when copying sigfigs)
        uint8 sigfigIndex;
        /// @dev index of decimal place (0 if no decimal)
        uint8 decimalIndex;
        /// @dev start index for trailing/leading 0's for very small/large numbers
        uint8 zerosStartIndex;
        /// @dev end index for trailing/leading 0's for very small/large numbers
        uint8 zerosEndIndex;
        /// @dev true if decimal number is less than one
        bool isLessThanOne;
        /// @dev true if string should include "%"
        bool isPercent;
    }
    ...
    function sigfigsRounded(uint256 value, uint8 digits) private pure returns (uint256, bool) {
        ...
        /// @dev 99999 -> 100000 gives an extra sigfig
        if (value == 100000) {
            value /= 10;
            extraDigit = true;
        }
    ...
    function fixedPointToDecimalString(
        ...
        if (priceBelow1) {
            /// @dev 10 ** 43 is precision needed to retreive 5 sigfigs of smallest possible price + 1 for rounding
            value = FullMath.mulDiv(value, 10**44, 1 << 128);
        } else {
            /// @dev leave precision for 4 decimal places + 1 place for rounding
        ...
        /// @dev don't count extra digit kept for rounding
        digits = digits - 1;

        /// @dev address rounding
        ...
        if (priceBelow1) {
            /// @dev 7 bytes ( "0." and 5 sigfigs) + leading 0's bytes
    ...
    function feeToPercentString(uint24 fee) internal pure returns (string memory) {
        ...
        while (feeDigits.temp != 0) {
            if (feeDigits.numSigfigs > 0) {
                /// @dev count all digits preceding least significant figure
        ...
        if (feeDigits.digits >= 5) {
            /// @dev if decimal > 1 (5th digit is the ones place)
