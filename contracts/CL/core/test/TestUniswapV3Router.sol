...
contract TestShadowV3Router is IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IShadowV3Pool(msg.sender).token0()
                : IShadowV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IShadowV3Pool(pools[0]).token1();
            IShadowV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IShadowV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IShadowV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
