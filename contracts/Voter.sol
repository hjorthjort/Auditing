...
contract Voter is IVoter, ReentrancyGuard, Initializable {
    ...
    mapping(address gauge => address feeDistributor) public feeDistributorForGauge;

    mapping(address pool => mapping(uint256 period => uint256 totalVotes)) public poolTotalVotesPerPeriod;
    mapping(address user => mapping(uint256 period => mapping(address pool => uint256 totalVote))) public
        userVotesForPoolPerPeriod;
    mapping(address user => mapping(uint256 period => address[] pools)) public userVotedPoolsPerPeriod;
    mapping(address user => mapping(uint256 period => uint256 votingPower)) public userVotingPowerPerPeriod;
    ...
    mapping(address gauge => mapping(uint256 period => bool distributed)) public gaugePeriodDistributed;

    mapping(address gauge => uint256 period) public lastDistro;

    mapping(address gauge => bool legacyGauge) public isLegacyGauge;
    mapping(address gauge => bool clGauge) public isClGauge;

    mapping(address => bool) public isWhitelisted;
    mapping(address => bool) public isAlive;

    /// @dev How many different CL pools there are for the same token pair
    mapping(address token0 => mapping(address token1 => int24[])) internal _tickSpacingsForPair;
    /// @dev what is the main tickspacing
    mapping(address token0 => mapping(address token1 => int24)) internal _mainTickSpacingForPair;
    /// @dev specific gauge based on tickspacing
    mapping(address token0 => mapping(address token1 => mapping(int24 tickSpacing => address gauge))) internal
        _gaugeForClPool;
    ...
    function _reset(address user) internal {
        ...
        if (votingPower > 0) {
            ...
            for (uint256 i; i < votedPools.length; ++i) {
                ...
                IFeeDistributor(feeDistributorForGauge[gaugeForPool[votedPools[i]]])._withdraw(userVote, user);
    ...
    function vote(address user, address[] calldata _pools, uint256[] calldata _weights) external {
        ...
        require(_vote(user, votedPools, _weights), Errors.VOTE_UNSUCCESSFUL());
    }

    function _vote(address user, address[] memory _pools, uint256[] memory _weights) internal returns (bool) {
    ...
    function _distribute(address _gauge, uint256 _claimable, uint256 _period) internal {
        ...
        if (isAlive[_gauge]) {
            ...
            if (_xShadowClaimable > 0) {
                if (_xShadowClaimable / DURATION == 0 || _xShadowClaimable < IGauge(_gauge).left(_xShadow)) {
                    canDistribute = false;
                }
            }
        ...
        returns (address)
    {
        ...
        address _feeDistributor = IFeeDistributorFactory(feeDistributorFactory).createFeeDistributor(_feeCollector);
        ...
        /// @dev mainTickSpacing logic
        {
            ...
            if (mainTickSpacing == 0) {
                ...
                address mainGauge = _gaugeForClPool[token0][token1][mainTickSpacing];
                poolRedirect[_pool] = poolForGauge[mainGauge];
                feeDistributorForGauge[_gauge] = feeDistributorForClGauge[mainGauge];
    ...
    /// @inheritdoc IVoter
    function setMainTickSpacing(address tokenA, address tokenB, int24 tickSpacing) external onlyGovernance {
        /// @dev sort the tokens
        (address token0, address token1) = _sortTokens(tokenA, tokenB);
        ...
        require(mainGauge != address(0), Errors.NO_GAUGE(mainGauge));
    ...
    function claimClGaugeRewards(
        ...
        RewardClaimers.claimClGaugeRewards(nfpManager, _gauges, _tokens, _nfpTokenIds);
    }
    ...
    function claimIncentives(address owner, address[] calldata _feeDistributors, address[][] calldata _tokens)
        external
    {
        RewardClaimers.claimIncentives(voteModule, owner, _feeDistributors, _tokens);
    }
    /// @inheritdoc IVoter
    ...
    function distribute(address _gauge) public nonReentrant {
        ...
        /// @dev if the last distribu$tion wasnt the current period
        if (_lastDistro != currentPeriod) {
            /// @dev check if a CL gauge
            if (isClGauge[_gauge]) {
                IShadowV3Pool poolV3 = IShadowV3Pool(pool);
