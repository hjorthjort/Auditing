...
contract PairFactoryTest is TestBase {
    ...
    function test_createPair() public {
        ...
        // Check that the default fee was set correctly
        assertEq(IPair(pair).fee(), 3000, "Default fee should be 0.30%");
        assertEq(IPairMinimal(pair).token0(), address(token0) < address(token1) ? address(token0) : address(token1), "token0 not set correctly");
        assertEq(IPairMinimal(pair).token1(), address(token0) < address(token1) ? address(token1) : address(token0), "token1 not set correctly");
        assertEq(IPairMinimal(pair).stable(), true, "stable flag not set correctly");
        assertEq(IPairMinimal(pair).factory(), address(factory), "factory address not set correctly");
    }

    function test_createPairRevertsForSameTokens() public {
        vm.expectRevert(abi.encodeWithSignature("Errors.INVALID_ASSORTMENT()"));
        factory.createPair(address(token0), address(token0), true);
    }

    function test_createPairRevertsForZeroAddress() public {
        vm.expectRevert(abi.encodeWithSignature("Errors.ZERO_ADDRESS()"));
    ...
    function test_createPairRevertsForExistingPair() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Errors.PAIR_EXISTS()"));
        factory.createPair(address(token0), address(token1), true);
    }
    ...
    function test_setFeeSplitWhenNoGauge() public {
        ...
        assertFalse(factory.feeSplitWhenNoGauge(), "Initial feeSplitWhenNoGauge should be false");
        ...
        // Verify effect on new pairs
        address pair = factory.createPair(address(token0), address(token1), true);
        assertEq(IPair(pair).feeRecipient(), factory.treasury(), "Treasury not set as fee recipient");
        assertEq(IPair(pair).feeSplit(), factory.feeSplit(), "FeeSplit not set correctly on new pair");
    }
    ...
    function test_setFeeSplit() public {
        ...
        uint256 initialFeeSplit = factory.feeSplit();
        ...
        assertEq(IPair(pair).feeSplit(), initialFeeSplit, "Pair feeSplit should remain unchanged");
    }
    ...
    function test_setSkimEnabled() public {
        ...
        // Verify initial state
        assertFalse(factory.skimEnabled(pair), "Initial skim status should be false");
    ...
    function test_createPairWithoutFeeSplitWhenNoGauge() public {
        // Ensure feeSplitWhenNoGauge is false
        assertFalse(factory.feeSplitWhenNoGauge(), "Initial feeSplitWhenNoGauge should be false");
        
        address pair = factory.createPair(address(token0), address(token1), true);
        
        // Verify pair state
        assertEq(IPair(pair).feeRecipient(), address(0), "Fee recipient should not be set");
        assertEq(IPair(pair).feeSplit(), 0, "FeeSplit should not be set");
    }
}
