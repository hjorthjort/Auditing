...
contract LauncherPluginTest is TestBase {
    ...
    function test_constructor() public view {
        assertEq(address(launcherPlugin.voter()), address(mockVoter), "Voter address mismatch");
        assertEq(launcherPlugin.accessHub(), address(accessHub), "AccessHub address mismatch");
        assertEq(launcherPlugin.operator(), alice, "Operator address mismatch");
    }
    ...
    function testFuzz_setConfigs(uint256 take, address recipient) public {
        ...
        assertEq(storedTake, take, "Take value mismatch");
        assertEq(storedRecipient, recipient, "Recipient address mismatch");
    }
    ...
    function test_enablePool() public {
        ...
        assertTrue(launcherPlugin.launcherPluginEnabled(mockPool), "Pool not enabled");
        assertEq(launcherPlugin.feeDistToPool(mockFeeDist), mockPool, "Fee distributor mapping incorrect");
    }
    ...
    function test_enablePoolNoFeeDist() public {
        ...
        // Enable pool
        vm.prank(address(accessHub));
        vm.expectRevert(abi.encodeWithSignature("NO_FEEDIST()"));
        launcherPlugin.enablePool(mockPool);

        // Verify pool is not enabled
        assertFalse(launcherPlugin.launcherPluginEnabled(mockPool), "Pool should not be enabled");
        assertEq(launcherPlugin.feeDistToPool(mockFeeDist), address(0), "Fee distributor mapping should be zero address");
    }
    ...
    function test_migratePool() public {
        ...
        // Verify migration
        assertTrue(launcherPlugin.launcherPluginEnabled(mockNewPool), "New pool not enabled");
        assertFalse(launcherPlugin.launcherPluginEnabled(mockPool), "Old pool still enabled");
        assertEq(launcherPlugin.feeDistToPool(mockNewFeeDist), mockNewPool, "Fee distributor mapping incorrect");

        // Verify configs were copied
        (uint256 take, address recipient) = launcherPlugin.poolConfigs(mockNewPool);
        assertEq(take, 100, "Take value not copied correctly");
        assertEq(recipient, alice, "Recipient not copied correctly");
    }
    ...
    function test_disablePool() public {
        ...
        // Verify state
        assertFalse(launcherPlugin.launcherPluginEnabled(mockPool), "Pool still enabled");
        (uint256 take, address recipient) = launcherPlugin.poolConfigs(mockPool);
        assertEq(take, 0, "Take not reset to 0");
        assertEq(recipient, address(0), "Recipient not reset to zero address");
        assertEq(launcherPlugin.feeDistToPool(mockFeeDist), address(0), "Fee distributor mapping not reset");
    }
    ...
    function test_setOperator() public {
        ...
        assertEq(launcherPlugin.operator(), bob, "Operator not updated");
    }
    ...
    function test_grantAuthority() public {
        ...
        assertTrue(launcherPlugin.authorityMap(bob), "Authority not granted");
    }
    ...
    function test_revokeAuthority() public {
        ...
        assertFalse(launcherPlugin.authorityMap(bob), "Authority not revoked");
    }
    ...
    function test_values() public {
        ...
        assertEq(take, 100, "Take value mismatch");
        assertEq(recipient, alice, "Recipient address mismatch");
    }
}
