...
contract XShadow is ERC20, IXShadow, Pausable {
    ...
    function rebase() external whenNotPaused {
        ...
        require(msg.sender == MINTER, Errors.NOT_MINTER());
        ...
        if (
            ...
            period > lastDistributedPeriod && pendingRebase >= BASIS
        ) {
    ...
    function createVest(uint256 _amount) external whenNotPaused {
        ...
        vestInfo[msg.sender].push(VestPosition(_amount, block.timestamp, block.timestamp + MAX_VEST, vestLength));
    ...
    function exitVest(uint256 _vestID) external whenNotPaused {
        ...
        require(_vest.amount != 0, Errors.NO_VEST());
        ...
        /// @dev linear calculations
            ...
            uint256 vestEarned =
                ((_amount * (BASIS - SLASHING_PENALTY) * (block.timestamp - _start)) / MAX_VEST) / BASIS;
    ...
    function rescueTrappedTokens(address[] calldata _tokens, uint256[] calldata _amounts) external onlyGovernance {
        for (uint256 i = 0; i < _tokens.length; ++i) {
            ...
            require(_tokens[i] != address(SHADOW), Errors.CANT_RESCUE());
