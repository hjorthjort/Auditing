...
contract AccessHubTest is TestBase {
    ...
    MockShadowV3Factory public shadowV3PoolFactory;
    ...
    function setUp() public override {
        ...
        shadowV3PoolFactory = new MockShadowV3Factory();
        ...
        IAccessHub.InitParams memory params = IAccessHub.InitParams({
            ...
            x33: address(mockX33),
            shadowV3PoolFactory: address(shadowV3PoolFactory),
        ...
        vm.label(address(shadowV3PoolFactory), "shadow_v3_factory");
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            if (concentrated[i]) {
                vm.mockCall(
                    ...
                    abi.encodeWithSelector(IShadowV3Factory.setFee.selector, pools[i], swapFees[i]),
                    abi.encode()
                );
            } else {
                vm.mockCall(
    ...
    function test_setFeeSplitCL() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            vm.mockCall(
                address(shadowV3PoolFactory),
                abi.encodeWithSelector(IShadowV3Factory.setPoolFeeProtocol.selector, pools[i], clFeeSplits[i]),
                abi.encode()
            );
        }
        accessHub.setFeeSplitCL(pools, clFeeSplits);
    ...
    function test_reviveGauge() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            vm.mockCall(
                address(shadowV3PoolFactory),
                abi.encodeWithSelector(IShadowV3Factory.feeProtocol.selector),
                abi.encode(5)
            );
            vm.mockCall(
                pools[i],
                abi.encodeWithSelector(IShadowV3Factory.setPoolFeeProtocol.selector, pools[i], 5),
                abi.encode()
            );
        }
        accessHub.reviveGauge(pools);
    ...
    function test_setNewTimelockRevertsForSameAddress() public {
        ...
        vm.expectRevert(Errors.SAME_ADDRESS.selector);
    ...
    function test_executeTimelockAccess() public {
        ...
        vm.startPrank(protocolOperator);
        vm.expectRevert(abi.encodeWithSelector(Errors.NOT_TIMELOCK.selector, protocolOperator));
        accessHub.execute(address(0), "");
        vm.stopPrank();

        vm.startPrank(TREASURY);
        vm.expectRevert(abi.encodeWithSelector(Errors.NOT_TIMELOCK.selector, TREASURY));
        accessHub.execute(address(0), "");
        vm.stopPrank();
    ...
    function test_setVoterAddressInFactoryV3() public {
        ...
        vm.mockCall(
            address(shadowV3PoolFactory),
            abi.encodeWithSelector(IShadowV3Factory.setVoter.selector, newVoter),
            abi.encode()
        );
        accessHub.setVoterAddressInFactoryV3(newVoter);
        vm.stopPrank();
    ...
    function test_executeFailure() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.MANUAL_EXECUTION_FAILURE.selector, abi.encodeWithSignature("failingFunction()")
            )
        );
