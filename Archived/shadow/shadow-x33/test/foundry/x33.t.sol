...
contract x33Test is TestBase {
    ...
    function setUp() public override {
        ...
        IAccessHub.InitParams memory params = IAccessHub.InitParams({
            ...
            x33: address(mockX33),
            shadowV3PoolFactory: address(CL_FACTORY),
    ...
    function test_claimIncentives() public {
        ...
        distributors[0] = address(0xfee1);
        tokens[0] = new address[](2);
        tokens[0][0] = address(xShadow);
        tokens[0][1] = address(shadow);

        // Mock the underlying calls that happen in RewardClaimers.claimIncentives()

        for (uint256 i; i < distributors.length; i++) {
            vm.mockCall(
                distributors[i],
                abi.encodeWithSelector(IFeeDistributor.getRewardForOwner.selector, address(vault), tokens[i]),
                abi.encode()
            );
        }
        ...
        vm.prank(address(TREASURY));
        vm.expectEmit(false, false, false, true);
        emit IX33.ClaimedIncentives(distributors, tokens);
    ...
    function testFuzz_swapIncentiveViaAggregator(uint256 amountIn, uint256 minAmountOut) public {
        ...
        emit IX33.SwappedBribe(address(TREASURY), tokenIn, amountIn, minAmountOut);
        vault.swapIncentiveViaAggregator(params);
        // 8. Verify balances after swap
        assertEq(IERC20(tokenIn).balanceOf(address(vault)), 0, "TokenIn balance should be 0 after swap");
        assertEq(vault.shadow().balanceOf(address(vault)), minAmountOut, "Shadow balance incorrect after swap");
    }
    ...
    function test_mintAndRatio() public {
        // 1. Unlock the vault
        vm.prank(address(TREASURY));
        vault.unlock();

        uint256 aliceShares = 100e18;
        uint256 bobShares = 200e18;

        // 2. Calculate assets needed for shares
        uint256 aliceAssets = vault.previewMint(aliceShares);
        uint256 bobAssets = vault.previewMint(bobShares);

        // 3. Give users xShadow
        deal(address(xShadow), alice, aliceAssets);
        deal(address(xShadow), bob, bobAssets);

        // 4. Alice mints and verify ratio
        vm.startPrank(alice);
        xShadow.approve(address(vault), aliceAssets);
        vault.mint(aliceShares, alice);
        vm.stopPrank();
        assertEq(vault.ratio(), 1e18, "Ratio should be 1e18 after Alice mint");

        // 5. Bob mints and verify ratio
        vm.startPrank(bob);
        xShadow.approve(address(vault), bobAssets);
        vault.mint(bobShares, bob);
        vm.stopPrank();
        assertEq(vault.ratio(), 1e18, "Ratio should be 1e18 after Bob mint");

        // 6. Alice redeems partially and verify ratio maintained
        vm.prank(alice);
        vault.redeem(aliceShares / 2, alice, alice);
        assertEq(vault.ratio(), 1e18, "Ratio should be 1e18 after Alice partial redeem");
    }

    function test_mintRequiresUnlocked() public {
        uint256 shares = 100e18;
        uint256 assets = vault.previewMint(shares);

        // 1. Give alice xShadow
        deal(address(xShadow), alice, assets);

        // 2. Try to mint while locked
        vm.startPrank(alice);
        xShadow.approve(address(vault), assets);

        vm.expectRevert(Errors.LOCKED.selector);
        vault.mint(shares, alice);
        vm.stopPrank();

        // 3. Unlock and verify mint now works
        vm.prank(address(TREASURY));
        vault.unlock();
    ...
    function testFuzz_mintRedeemRatioInvariant(uint256[] calldata shares) public {
        // 1. Bound array length
        ...
        // 3. Process each share amount
        for (uint256 i = 0; i < shares.length; i++) {
            // 4. Bound each share amount to reasonable range
            uint256 shareAmount = bound(shares[i], 1e18, 1000000e18);
            uint256 assetAmount = vault.previewMint(shareAmount);

            // 5. Give alice tokens and mint shares
            deal(address(xShadow), alice, assetAmount);
            vm.startPrank(alice);
            xShadow.approve(address(vault), assetAmount);
            vault.mint(shareAmount, alice);
            vm.stopPrank();

            totalShares += shareAmount;

            // 6. Verify ratio and supply after each mint
            assertEq(vault.ratio(), 1e18, "Ratio should be 1e18 after each mint");
            assertEq(vault.totalSupply(), totalShares, "Total supply should match cumulative shares");
        }
    }
}
