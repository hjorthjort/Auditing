...
describe("Voter", () => {
    ...
    describe("#initialization", () => {
        it("initialization", async () => {
            ...
            ).equal(await c.shadow.getAddress());
    ...
    describe("#killGauge", () => {
        ...
        describe("Normal case - legacy gauges", () => {
            ...
            it("no votes on pool", async () => {
                const tokensBefore = await c.shadow.balanceOf(
                    deployer.address,
                );
                ...
                expect(
                    await c.shadow.balanceOf(deployer.address),
    ...
    describe("#stuckEmissionsRecovery", () => {
        ...
        describe("#recovery on killed gauges", () => {
            ...
            it("normal operation", async () => {
                const period = await voter.getPeriod();
                const balanceBefore = await c.shadow.balanceOf(
                    voter.getAddress(),
                );
                ...
                const balanceAfter = await c.shadow.balanceOf(
                    voter.getAddress(),
                );
                ...
                expect(
                    await c.shadow.balanceOf(voter.getAddress()),
    ...
    describe("#setMainTickSpacing", () => {
        let clPool: typechain.ShadowV3Pool;
        let clGauge: typechain.GaugeV3;
        let otherClPool: typechain.ShadowV3Pool;
    ...
    describe("#vote, poke, reset", () => {
        ...
        it("mismatched lengths", async () => {
            ...
            ).to.be.revertedWithCustomError(voter, "LENGTH_MISMATCH");
    ...
    describe("#createGauge, createCLGauge", () => {
        let pair: typechain.Pair;
        let clPool: typechain.ShadowV3Pool;
        let gauge: typechain.Gauge;
        let clGauge: typechain.GaugeV3;
    ...
    describe("#claimClGaugeRewards, claimIncentives, claimRewards", () => {
        ...
        it("#claimClGaugeRewards", async () => {
            ...
            const ramBalanceBefore = await c.shadow.balanceOf(
                deployer.address,
            );
    ...
    describe("#notifyRewardAmount", () => {
        it("should revert other than for minter", async () => {
            ...
            await c.shadow.transfer(c.minter.getAddress(), 1n);
            await c.shadow
    ...
    describe("#distribute, distributeForPeriod", () => {
        ...
        it("normal operation on distribute for legacy gauge", async () => {
            ...
            const rewardsBefore = await c.shadow.balanceOf(c.gauge);
            ...
            expect(
                (await c.shadow.balanceOf(c.gauge)) - rewardsBefore,
        ...
        it("normal operation on distribute for cl gauge", async () => {
            ...
            const rewardsBefore = await c.shadow.balanceOf(c.clGauge);
            ...
            expect(
                (await c.shadow.balanceOf(c.clGauge)) - rewardsBefore,
                "distributed emissions amount is wrong",
            ).eq(emissions / 2n); // divide by two because the two pools have equal votes
        });
        ...
        it("normal operation on distributeForPeriod", async () => {
            ...
            let rewardsBefore = await c.shadow.balanceOf(c.clGauge);
            ...
            emissions = await c.minter.weeklyEmissions();
            rewardsBefore = await c.shadow.balanceOf(c.clGauge);
            ...
            expect(
                await c.shadow.balanceOf(c.clGauge),
    ...
    describe("#edge cases", () => {
        ...
        it.skip("if there isn't enough claimable to distribute", async () => {
            ...
            await voter.distribute(c.gauge.getAddress());
            expect(
                await c.shadow.balanceOf(c.gauge.getAddress()),
