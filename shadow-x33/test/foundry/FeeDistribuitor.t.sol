...
contract FeeDistributorTest is TestBase {
    ...
    function test_getPeriodReward() public {
        ...
        // Setup deposit, simulates voting power in the next period
        vm.prank(address(mockVoter));
        ...
        assertEq(
            ...
            0,
            "Last claim period should not be updated getPeriodReward does not update it"
        );
    ...
    function test_getRewardWithNewPeriodRewards() public {
        uint256 depositAmount = 100e18;
        uint256 rewardAmount = 1000e18;
        uint256 secondRewardAmount = 500e18;
        ...
        // Claim first period rewards
        uint256 balanceBefore = IERC20(address(token0)).balanceOf(alice);
        vm.startPrank(alice);
        feeDistributor.getReward(alice, tokens);
        uint256 balanceAfter = IERC20(address(token0)).balanceOf(alice);

        assertEq(balanceAfter - balanceBefore, rewardAmount, "Incorrect balance after first claim");
        assertEq(feeDistributor.earned(address(token0), alice), 0, "Incorrect earned after first claim");

        // Setup second period reward
        vm.startPrank(address(feeRecipient));
        deal(address(token0), address(feeRecipient), secondRewardAmount);
        IERC20(address(token0)).approve(address(feeDistributor), secondRewardAmount);
        feeDistributor.notifyRewardAmount(address(token0), secondRewardAmount);
        vm.stopPrank();
        // Mock deposit for new period
        vm.prank(address(mockVoter));
        feeDistributor._deposit(depositAmount, alice);
        ...
        // Check earned amount for new period
        uint256 earnedAmount = feeDistributor.earned(address(token0), alice);
        assertEq(earnedAmount, secondRewardAmount, "Incorrect earned amount for new period");
    }
