...
contract VoterTest is TestBase {
    ...
    function test_whitelistTokenSucceeds() public {
        ...
        emit IVoter.Whitelisted(address(accessHub), token);
    ...
    function test_revokeWhitelist() public {
        ...
        emit IVoter.WhitelistRevoked(address(accessHub), token, true);
    ...
    function testFuzz_notifyRewardAmount(uint256 amount) public {
        ...
        vm.expectEmit(true, true, false, true);
        emit IVoter.NotifyReward(address(mockMinter), address(shadow), amount);
    ...
    function test_reviveGaugeEmitsGaugeRevivedEvent() public {
        ...
        vm.prank(address(accessHub));
        vm.expectEmit(true, false, false, false);
        emit IVoter.GaugeRevived(gauge);
        voter.reviveGauge(gauge);
    }
    ...
    function test_setMainTickSpacingEmitsMainTickSpacingChangedEvent() public {
        ...
        emit IVoter.MainTickSpacingChanged(token0, token1, tickSpacing);
        voter.setMainTickSpacing(tokenA, tokenB, tickSpacing);
    }
    ...
    function test_setGovernorSucceedsWhenCallerIsGovernance() public {
        ...
        vm.prank(address(accessHub));
        vm.expectEmit(true, true, false, false);
        emit IVoter.NewGovernor(address(accessHub), newGovernor);
    ...
    function mockClGaugeCalls(address tokenA, address tokenB, int24 tickSpacing) internal returns (address pool) {
        ...
        pool = address(new MockShadowV3Pool());
        vm.mockCall(
            CL_FACTORY,
            abi.encodeWithSelector(IShadowV3Factory.getPool.selector, address(tokenA), address(tokenB), tickSpacing),
            abi.encode(pool)
        );

        // Step 2: Mock slot0 call to indicate pool is initialized
        vm.mockCall(pool, abi.encodeWithSelector(ShadowV3Pool.slot0.selector), abi.encode(0, 0, 0, 0, 0, 0, true));

        // Step 3: Mock getFeeCollector call
        vm.mockCall(
            CL_FACTORY,
            abi.encodeWithSelector(IShadowV3Factory.feeCollector.selector),
        ...
        vm.mockCall(
            CL_FACTORY, abi.encodeWithSelector(IShadowV3Factory.gaugeFeeSplitEnable.selector, pool), abi.encode()
        );
    }
