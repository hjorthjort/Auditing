...
contract VoteModuleTest is TestBase {
    ...
    function test_rewardDustAccumulation() public {
        // Setup initial deposit
        vm.prank(alice);
        voteModule.deposit(100e18);
        ...
        deal(address(shadow), address(xShadow), 500e18);
        shadow.approve(address(voteModule), 500e18);
        voteModule.notifyRewardAmount(500e18);
        vm.stopPrank();

        // Wait for rewards to fully distribute
        ...
        // Check for dust (unclaimed rewards) in contract
        uint256 remainingShadow = shadow.balanceOf(address(voteModule));
        assertGt(remainingShadow, 0, "Should have some dust accumulated");
        assertLt(remainingShadow, 1e18, "Dust should be less than 1 token");

        // Verify no more rewards are claimable
        assertEq(voteModule.earned(alice), 0, "No more rewards should be claimable");
        assertEq(voteModule.left(), 0, "No rewards should be left to distribute");
    }
